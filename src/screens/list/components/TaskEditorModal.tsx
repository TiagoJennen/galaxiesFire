// Bewerkmodal voor hoofdtaak met deadline-, media- en locatiesturing.
...
export const TaskEditorModal: React.FC<TaskEditorModalProps> = ({
  ...
  const deadlineValue = deadlinePreview?.trim().length
    ? deadlinePreview
    : strings.noDeadline;
  // Toon fallback tekst wanneer geen deadline gekoppeld is.
  const deadlineEmpty = !(deadlinePreview && deadlinePreview.trim().length);
  const resolvedDeadlineISO = deadlineISO ?? editingTodo.deadline ?? null;
  let isDeadlineOverdue = false;
  if (resolvedDeadlineISO && !editingTodo.done) {
  ...
  // Bepaal welke locatiestring we tonen; blanco waardes krijgen placeholder.
  const locationEmpty = !(
        ...
        <View style={styles.panelWrapper}>
          <View style={styles.panel}>
            <View style={styles.headerRow}>
              <Text style={styles.title}>{strings.editTask}</Text>
              <Pressable
                onPress={onClose}
                accessibilityRole="button"
                accessibilityLabel={strings.cancel}
            ...
            >
              <View style={styles.sectionCard}>
                <Text style={styles.sectionTitle}>{strings.taskName}</Text>
                <TextInput
                  value={taskText}
